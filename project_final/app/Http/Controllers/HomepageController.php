...
class HomepageController extends Controller
{
    ...
    public function deleteCart(Request $request)
    {
        $userId = Auth::user()?->id_user;
        if (!$userId) {
            return redirect()->route('login')->withErrors('Musisz być zalogowany.');
        }   
        ...
        if (!$order) {
            return redirect()->route('cart')->withErrors('Nie masz aktywnego zamówienia.');
        }

        $productId = $request->input('product_id_product');

        if (!$productId) {
            return redirect()->route('cart')->withErrors('Brak danych produktu.');
        }
        ...
        if (!$productOrder) {
            return redirect()->route('cart')->withErrors('Produkt nie znaleziony.');
        }

        $product = Product::find($productId);
        if (!$product) {
            return redirect()->route('cart')->withErrors('Produkt nie istnieje.');
        }
        ...
        if ($productOrder->quantity <= 1) {
            ...
            ProductOrder::where('order_id_order', $order->id_order)
                ...
                $newQuantity = $productOrder->quantity - 1;
                $newPrice = $newQuantity * $product->price;
        ...
        return redirect()->route('cart')->with('success', 'Zaktualizowano ilość produktu w koszyku.');
    }

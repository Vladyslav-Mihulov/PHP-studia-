...
<form action="<?php print(_APP_ROOT); ?>/app/kredyt.php" method="post" class="pure-form pure-form-stacked">
	...
	<fieldset>
		<label for="id_kwota">Wpisz kwote: </label>
		<input id="id_kwota" type="text" name="kwota" value="<?php out($kwota) ?>" />
                
		<label for="id_oprocentowanie">Wpisz orocentrowanie bez znaku %: </label>
		<input id="id_oprocentowanie" type="text" name="oprocentowanie" value="<?php out($oprocentowanie) ?>" />
                
		<label for="id_okres">Wpisz okres w latach </label>

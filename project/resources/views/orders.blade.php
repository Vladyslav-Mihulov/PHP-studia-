    ...
    <div class="inner">
        ...
        @else
            @foreach($ord as $order)
                ...
                <p>Data zamówienia: {{ \Carbon\Carbon::parse($order->date_order)->format('d.m.Y H:i') }}</p>
                
                @if($order->date_end_order)
                    <p>Data zakończenia: {{ \Carbon\Carbon::parse($order->date_end_order)->format('d.m.Y H:i') }}</p>
                @else
